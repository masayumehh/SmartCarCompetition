...
/**
 * @brief 读取稳定边在当前行的边界�? * @param stable_side 稳定边侧
 * @param left_edges 左边界数�? * @param right_edges 右边界数�? * @param row 当前行号
...
/**
 * @brief 读取异常边在当前行的边界�? * @param stable_side 稳定边侧
 * @param left_edges 左边界数�? * @param right_edges 右边界数�? * @param row 当前行号
 ...
 * @return uint8_t 返回 1 表示当前行稳定边仍可�? */
...
/**
 * @brief 判断当前帧是否已恢复为正常直�? *
 * 当环岛结束后，图像上部和中部会重新出现：
 * 1. 左右边线都完整；
 * 2. 宽度回到正常范围�? * 3. 中心波动明显减小�? *
 * @param left_edges 左边界数�? * @param right_edges 右边界数�? * @param centers 中心数组
 * @param valid_rows 有效行标�? * @return uint8_t 返回 1 表示当前帧更像正常直�? */
...
/**
 * @brief 基于稳定边侧检测单帧环岛候�? *
 * 识别逻辑按你的思路拆成三步�? * 1. 先确定一侧边线稳定、单调，可作为参考边�? * 2. 再在另一侧寻�?A 点和 B 点；
 * 3. 最后继续向上寻�?C 点，得到完整入口几何�? *
 * @param stable_side 假设的稳定边�? * @param left_edges 左边界数�? * @param right_edges 右边界数�? * @param valid_rows 有效行标�? * @param left_ref 下部参考左边界
 * @param right_ref 下部参考右边界
 * @param width_ref 下部参考宽�? * @param guide_center 下部平均中心
 * @return Roundabout_Candidate 当前稳定边假设下的候选结�? */
                                                            ...
                                                            int16_t guide_center)
{
        ...
        if (phase == 1U)
        {
                ...
                abs_diff_u16((uint16_t)abnormal_edge, (uint16_t)abnormal_ref) >= ROUNDABOUT_B_POINT_MIN_OFFSET)
            {
                ...
                if (recover_streak >= ROUNDABOUT_RECOVER_MIN_ROWS)
                {
                    ...
                    candidate.point_c = candidate.point_b;         /* C 点先�?B 点起步�?*/
    ...
    if (abs_diff_u16((uint16_t)candidate.point_b.col, (uint16_t)candidate.point_a.col) < ROUNDABOUT_B_POINT_MIN_OFFSET) return candidate; /* A �?B 偏移不够�?*/
    if (abs_diff_u16((uint16_t)candidate.point_c.col, (uint16_t)candidate.point_b.col) < ROUNDABOUT_C_POINT_MIN_OFFSET) return candidate; /* B �?C 偏移不够�?*/

    candidate.arc_peak = candidate.point_c.col;  /* C 点列号同时作为弧顶位置输出�?*/

    if (stable_side == ROUNDABOUT_SIDE_LEFT)
    {
        candidate.supplement_width = (uint16_t)(candidate.point_c.col - left_ref);  /* 顺时针时�?C 点到稳定左边界的距离作为补线宽度�?*/
    }
    else
    {
        candidate.supplement_width = (uint16_t)(right_ref - candidate.point_c.col); /* 逆时针时用稳定右边界�?C 点的距离作为补线宽度�?*/
    }
...
/**
 * @brief 用候选结果刷新状态机上下�? * @param candidate 当前帧环岛候�? */
 ...
 * 控制逻辑分三段：
 * 1. A-B：入口第一段直线补线；
 * 2. B-C：继续向环岛弧线过渡�? * 3. C 以上：使用稳定边拾边，并按补线宽度外推另一侧�? *
 * �?EXITING 状态下，会把环岛中线与普通中线做平滑混合�? * 让车辆更顺地切回普通巡线�? */
...
/**
 * @brief 获取当前环岛状�? * @return Roundabout_State 当前状态机状�? */
